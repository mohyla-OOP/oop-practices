...
CopyVector::CopyVector(const CopyVector &v) : _n(v._n), _v(new double[_n]), _id(++_freeId)
{
  ...
  ++counter;
#endif
  for (size_t k = 0; k < _n; ++k) _v[k] = v._v[k];
}

size_t CopyVector::size() const { return _n; }
...
double CopyVector::operator[](size_t k) const { return _v[k]; }

double &CopyVector::operator[](size_t k) { return _v[k]; }
