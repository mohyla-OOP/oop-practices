...
Vector::Vector(size_t n) : _n(n), _v(new double[_n]), _id(++_freeId)
{
  ...
  for (size_t k = 0; k < _n; ++k) _v[k] = k + _id;
}
Vector::~Vector()
{
  delete[] _v;
...
size_t Vector::size() const { return _n; }
...
double Vector::operator[](size_t k) const { return _v[k]; }

double &Vector::operator[](size_t k) { return _v[k]; }
