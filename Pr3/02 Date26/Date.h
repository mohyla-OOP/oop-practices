...
constexpr int to_int(Month m) noexcept { return static_cast<int>(m); }
...
class Date
{
 public:
  ...
  // Форматування: "YYYY-MM-DD"
  std::string to_string() const { return std::format("{:04}-{:02}-{:02}", _y, to_int(_m), _d); }
  ...
  Date &add_days(int delta)
  {
    ...
    if (delta > 0)
    {
      while (delta > 0)
      {
        ...
        {
          _m = static_cast<Month>(to_int(_m) + 1);
        }
      }
    }
    else
  ...
  friend bool operator==(const Date &a, const Date &b) noexcept { return a._y == b._y && a._m == b._m && a._d == b._d; }
...
inline std::ostream &operator<<(std::ostream &os, const Date &d) { return os << d.to_string(); }
