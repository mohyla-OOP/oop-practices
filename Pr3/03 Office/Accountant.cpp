...
Accountant::Accountant(const Employer &employer, const Person &accountant) : _myEmployer(employer), _who(accountant)
{
  cout << "Accountant " << _who.getName() << " is hired" << endl;
};

Accountant::~Accountant() { cout << "Accountant was deleted" << endl; }
...
void Accountant::payroll() const
{
  ...
  for (int i = 0; i < _myEmployer._volume; i++)
    ...
    if (!_myEmployer.free(i)) payroll(_myEmployer._pemployee(i));
...
*/
Accountant::Accountant(const Accountant &p) : _myEmployer(p._myEmployer), _who(p._who)
{
