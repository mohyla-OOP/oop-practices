...
class Aggregate
{
 ...
 public:
  ...
  Aggregate(const Aggregate &other) : p_(other.p_ ? new Aggregee(*other.p_) : nullptr) {}
...
Aggregate ah() { return std::move(Aggregate()); }
...
MoveOnly fMoveOnly() { return MoveOnly(); }
...
class AggregateUnique
{
 ...
 public:
  ...
  explicit AggregateUnique(const Aggregee &a) : p_(std::make_unique<Aggregee>(a)) {}

  explicit AggregateUnique(const Aggregee *a) : p_(std::make_unique<Aggregee>(*a)) {}

  // Rule of Zero: no manual dtor/copy/move needed.
  // Copying semantics can be defined explicitly if you want deep copy:
  AggregateUnique(const AggregateUnique &other) : p_(other.p_ ? std::make_unique<Aggregee>(*other.p_) : nullptr) {}
