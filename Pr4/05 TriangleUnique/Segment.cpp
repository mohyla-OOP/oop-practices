...
Segment::Segment(const double x1, const double y1, const double x2, const double y2) : _a(x1, y1), _b(x2, y2), myId(++freeID)
{
  ++_call;
#ifndef NDEBUG
...
Segment::Segment(const Point &a, const Point &b) : _a(a), _b(b), myId(++freeID)
{
  ++_call;
#ifndef NDEBUG
  out << myId << ": a Segment was created: " << *this;
...
double Segment::distance(const Point &c) const
{
  ...
  if (B == 0) return fabs(-C / A - c.y());
  if (A == 0) return fabs(-C / B - c.x());
