...
Triangle::Triangle(const Point &a, const Point &b, const Point &c) : a_(a), b_(b), c_(c), ab_(nullptr), bc_(nullptr), ca_(nullptr) { ++call_count_; }

// Copy-construct: copy vertices; do not copy the side caches (lazy rebuild)
Triangle::Triangle(const Triangle &rhs) : a_(rhs.a_), b_(rhs.b_), c_(rhs.c_), ab_(nullptr), bc_(nullptr), ca_(nullptr) { ++call_count_; }
...
std::ostream &operator<<(std::ostream &os, const Triangle &t) { return t.show(os); }