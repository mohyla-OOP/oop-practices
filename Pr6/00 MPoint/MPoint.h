...
{
  int n = p.size();
  return Point{centre(p, n, 0), centre(p, n, 1)};
}
...
{
  int n = p.size();
  return {centre(p, n, &Point::_x), centre(p, n, &Point::_y)};
}

std::ostream &operator<<(std::ostream &os, const Point &a) { return os << '(' << a._x << ',' << a._y << ')'; }
