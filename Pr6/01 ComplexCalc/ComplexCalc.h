...
Operation ops[] = {{"+=", &Complex::operator+= },
                    {
                        "-=",
                        &Complex::operator-= },
};
...
const std::map<std::string, PMF> optable = {{"+=", &Complex::operator+= }, {"-=", &Complex::operator-= }, {"*=", &Complex::operator*= } };
...
{
  if (auto it = optable.find(opname); it != optable.end()) return (a.*it->second)(b);
