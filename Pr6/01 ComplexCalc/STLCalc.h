...
using CPtr = SComplex &(SComplex::*const) (const SComplex &);
...
class STLCalc
{
 ...
 public:
  STLCalc(const SComplex &a = 0, const SComplex &x = 0) : _a(a), _x(x) {};
...
const std::map<std::string, SPMF> optableSTL = {{"+=", &SComplex::operator+= }, {"-=", &SComplex::operator-= }, {"*=", &SComplex::operator*= } };
#include <map>
#include <string>
...
{
  if (auto it = optableSTL.find(opname); it != optableSTL.end()) return (a.*it->second)(b);
