...
inline double select(const MPoint &u, const MPoint::ConstCoord coord) { return u._p.*coord; }
inline void modify(MPoint &u, MPoint::Coord coord, double a)
{
  u._p.*coord = a;
  return;
}
...
// Center of mass
Point centre(const MPoint p[], const size_t n) { return {centre(p, n, &Point::_x), centre(p, n, &Point::_y)}; }

ostream &operator<<(ostream &os, const Point &a) { return os << '(' << a._x << ',' << a._y << ')'; }
